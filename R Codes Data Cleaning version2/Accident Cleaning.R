...
#To set up your API key, please follow:
...
ggplot(data = accident.dt, aes(x = number_of_vehicles,fill=number_of_vehicles)) +
  ...
  labs(x = "Number of vehicles", 
       y = "Count", 
       title = "Count of Serious vs Not Serious accident acros different number of vehicles") +
...
#Further data exploration, since no perfect correlation exists.
...
ggplot(data = accident.dt, aes(x = carriageway_hazards,group=accident_severity)) +
  ...
  facet_grid(.~accident_severity)
#There are too many categories of carriageway hazards...

#Lets try regrouping into a binary categorical variable to make the data more meaningful
levels(accident.dt$carriageway_hazards) = list("None" = 0, 
                                               "Presence of carriageway hazards" = c(1,2,3,6,7))
...
ggplot(data = accident.dt, aes(x = special_conditions_at_site,group=accident_severity)) +
  ...
  facet_grid(.~accident_severity)
#The cardinality for special on site conditions is too high

#Once again, regrouping into binary categorical variable
levels(accident.dt$special_conditions_at_site) = list("None" = 0, 
                                                      "Presence of special conditions" = c(1:7))
...
ggplot(data = accident.dt, aes(x = speed_limit,group=accident_severity)) +
  ...
  labs(fill = "speed_limit", 
       x = "Speed Limit",
       y = "Percentage", 
       title = "Accident Severity different speed limits") +
  facet_grid(.~accident_severity)
