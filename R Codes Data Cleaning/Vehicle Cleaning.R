...
vehicle_type[vehicle.dt$vehicle_type == 2|vehicle.dt$vehicle_type == 3|
               vehicle.dt$vehicle_type == 4|vehicle.dt$vehicle_type == 5] <- "Motorcycle"
vehicle_type[vehicle.dt$vehicle_type == 8] <- "Taxi/Private hire car"
vehicle_type[vehicle.dt$vehicle_type == 9] <- "Car"
vehicle_type[vehicle.dt$vehicle_type == 10] <- "Minibus (8-16 seats)"
vehicle_type[vehicle.dt$vehicle_type == 11] <- "Bus or coach (>17 seats)"
...
vehicle_type <- factor(vehicle_type, levels = c("Motorcycle", "Taxi/Private hire car", 
                                                "Car", "Minibus (8-16 seats)", 
                                                "Bus or coach (>17 seats)", 
                                                "Agricultural vehicle", "Van/Goods <=3.5t mgw", 
                                                "Goods >3.5t & <7.5t", "Goods >=7.5t", 
                                                "Electric motorcycle", "Other vehicle"))
...
cramerV(vehicle.dt$accident_severity, vehicle.dt$age_group, bias.correct = T) #0.03838
cramerV(vehicle.dt$accident_severity, vehicle.dt$home_area, bias.correct = T) #0.03851
cramerV(vehicle.dt$accident_severity, vehicle.dt$imd_decile, bias.correct = T) #0.02049
cramerV(vehicle.dt$accident_severity, vehicle.dt$propulsion_code, bias.correct = T) #0.03555
cramerV(vehicle.dt$accident_severity, vehicle.dt$sex, bias.correct = T) #0.06614
...
#Exploring variables with stronger correlation

#===============================================================================

ggplot(data = vehicle.dt, aes(x = vehicle_type, fill = accident_severity)) + 
  geom_bar(position = "dodge") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + 
  labs(x = "Vehicle Type", 
       y = "Count", 
       title = "Accident severity by Vehicle Type")
#Accidents involving motorcycles appear to have a higher proportion of serious
#compared to non-serious.

ggplot(data = vehicle.dt, aes(x = imd_decile,fill = imd_decile)) +
  geom_bar() +
...
levels(vehicle.dt$imd_decile) = list("Most Deprived(0-20%)" = c(1,2), "More Deprived(20-40%)" = c(3,4), 
                                     "Moderately Deprived(40-50%)"= c(5,6), 
                                     "Less Deprived(20-40%)" = c(7,8), "Least Deprived(0-20%)" = c(9,10))

ggplot(data = vehicle.dt, aes(x = vehicle_manoeuvre, fill = accident_severity)) + 
  geom_bar(position = "dodge") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + 
  labs(x = "Manoeuvre Type", 
       y = "Count", 
       title = "Accident severity by Manoeuvre Type")

table(vehicle.dt$vehicle_manoeuvre)*100/length(vehicle.dt$vehicle_manoeuvre)

#Regroup manoeuvres into broader categories as some categories have barely any
#observations. Not necessary to have directional details.
levels(vehicle.dt$vehicle_manoeuvre) = list("Changing lanes" = c("Changing lane to left", "Changing lane to right"), 
                                            "Overtaking" = c("Overtaking moving vehicle - offside", 
                                                             "Overtaking static vehicle - offside", 
                                                             "Overtaking - nearside"), 
                                            "Going ahead left/right bend" = c("Going ahead left-hand bend", 
                                                                              "Going ahead right-hand bend"), 
                                            "Turning left" = c("Turning left", "Waiting to turn left"), 
                                            "Turning right" = c("Turning right", "Waiting to turn right"), 
                                            "Reversing" = "Reversing", "Parked" = "Parked", "Waiting to go - held up" = "Waiting to go - held up", 
                                            "Slowing or stopping" = "Slowing or stopping", "Moving off" = "Moving off", 
                                            "U-turn" = "U-turn", "Going ahead other" = "Going ahead other")
#Factor with 12 levels

ggplot(data = vehicle.dt, aes(x = first_point_of_impact, fill = accident_severity)) + 
  geom_bar(position = "dodge") + labs(x = "First point of impact", 
                                      y = "Count", 
                                      title = "Count of accidents by first point of impact")
#From observation, rear impact results in a much lower proportion of serious accidents
#compared to the other directions of impact.

ggplot(data = vehicle.dt, aes(x = skidding_and_overturning, fill = accident_severity)) + 
  geom_bar(position = "dodge")
table(vehicle.dt$skidding_and_overturning)*100/length(vehicle.dt$skidding_and_overturning)

table(vehicle.dt$vehicle_leaving_carriageway)
ggplot(data = vehicle.dt, aes(x = vehicle_leaving_carriageway, fill = accident_severity)) + 
  geom_bar() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

#Regroup into broad categories to reduce number of categories since number of
#observations for some categories is so low
levels(vehicle.dt$vehicle_leaving_carriageway) = list("Did not leave carriageway" = "Did not leave carriageway", 
                                                      "Nearside" = c("Nearside", "Nearside and rebounded"), 
                                                      "Offside" = c("Offside on to central reservation", 
                                                                    "Offside on to central res + rebounded", 
                                                                    "Offside - crossed central reservation", 
                                                                    "Offside", "Offside and rebounded"), 
                                                      "Straight ahead at junction" = "Straight ahead at junction")
